...
interface ThingDao : BaseDao<Thing> {
    ...
    @Query("select * from voucher where is_deleted = 0 and thing_id = :thingId order by date_at desc, time_at desc")
    fun pagingVoucherByThing(thingId: Long): PagingSource<Int, Voucher>
