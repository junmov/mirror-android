...
interface BudgetDao : BaseDao<Budget> {
    ...
    @Query("select * from budget where month_at = :month and account_id in (:accountIds)")
    suspend fun findAllByAccountId(accountIds: List<Long>, month: YearMonth): List<Budget>
}